...
describe("MarketplaceSection", () => {
  it("presents editable curated profiles without claiming the catalog is live", async () => {
    ...
    expect(screen.getAllByRole("button", { name: /creator profile coming soon/i })).toHaveLength(marketplaceProfiles.length);
    expect(screen.getByText(/free profiles remain open to everyone/i)).toBeInTheDocument();
