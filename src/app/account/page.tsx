...
export default function AccountPage() {
  return <LegalShell eyebrow={`${currentLaunch.stage.toUpperCase()} / OPTIONAL ACCOUNT`} title={currentLaunch.accounts ? "Sign in without turning your node into a cloud service." : "Accounts are not active in Preview."} summary="Community local operation never requires an account. Commercial accounts exist only for optional Pro presentation and hosted conveniences.">
    ...
    {currentLaunch.accounts ? <AuthPanel /> : <p><strong>Coming with the Commercial stage.</strong> No sign-in request is accepted by the Preview deployment.</p>}
    <h2>Account recovery and deletion</h2><p>Commercial activation requires tested account linking, email and GitHub recovery, session revocation, data export, deletion, subscription handling, and an auditable support path.</p>
  </LegalShell>;
}
