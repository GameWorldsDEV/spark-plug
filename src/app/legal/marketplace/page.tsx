...
export default function MarketplaceRulesPage() {
  return <LegalShell eyebrow="CREATOR MARKETPLACE RULES / DRAFT" title="Payment never replaces proof, review, or a safe way to report problems." summary="Everyone may use approved free listings. Planned Pro creators may submit free or paid assets, but publication is moderated and never automatic.">
    ...
    <h2>Sales remain disabled today</h2><p>The preview has no marketplace checkout, seller onboarding, payout, or paid placement. Before activation, every creator must accept seller terms and complete Stripe Connect onboarding. Every paid listing must show its creator, contents, price, the {MARKETPLACE_PLATFORM_FEE_PERCENT}% GameWorlds platform fee, separate payment costs, license, compatibility, refund boundary, and support expectations.</p>
    <h2>Required compatibility evidence</h2><p>Every paid technical asset must identify the computer maker and model, operating system, architecture, memory, applicable GPU and VRAM, engine version, model revision, and test date for each supported setup. Creators may not replace specific testing with universal compatibility claims.</p>
    <h2>Support and escalation</h2><p>Creators own first-line questions, troubleshooting, and reasonable revisions. Unresolved defects, abandonment, unsafe conduct, and refund requests escalate to GameWorlds administrators with the listing version and support history. See the <Link href="/legal/refunds">Marketplace Support &amp; Refund Policy</Link>.</p>
