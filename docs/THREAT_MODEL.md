...
signed entitlement snapshots, free public profile marketplace, forum, and
Stripe subscriptions. It excludes the private/local product except at the explicit manifest
handoff boundary.

## Assets and adversaries

Protected assets are account identity, subscription state, free profile
manifests, signing/service/webhook keys, moderation provenance,
forum integrity, and owner trust. Relevant adversaries include anonymous
scrapers, abusive signed-in users, malicious publishers, forged webhook
...
| Exfiltrate an unapproved manifest through catalog/RLS | Metadata-only catalog; manifest column grants revoked; delivery RPC checks publication state | Disposable-project RLS tests as anon, owner, non-owner, moderator, and service roles |
...
| Browser changes price, owner, or entitlement | Fixed server price allowlist; service-only mutations; webhook projection | Checkout and portal routes remain disabled until tested |
...
- Authorization failures for unavailable content do not reveal moderation or
  ownership state.
...
manual review of every public binary and route. Checkout, profile publication,
and forum write APIs remain Commercial-stage blockers until their endpoint
